...
file = ('/home/sourave/6galaxies/AGC9500/9500_masked.fits')
hdu = fits.open(file)
img = hdu[0].data
df=pd.DataFrame(img)
...
plt.clf()
ngauss = 11 
s = sectors_photometry(img, f.eps, f.theta, f.xpeak, f.ypeak,n_sectors=7,plot=1,minlevel=minlevel)
...
mge_print_contours(img, f.theta, xc, yc, m.sol,scale=scale,binning=5,sigmapsf=sigmapsf)
