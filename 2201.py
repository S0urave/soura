...
import pandas as pd
file = ('/home/sourave/6galaxies/AGC220901/220901_masked.fits')
hdu = fits.open(file)
img = hdu[0].data
df=pd.DataFrame(img)
...
scale = 0.3957 
sigmapsf = 1.3
f=find_galaxy(img,plot=1,nblob=1,fraction = 0.03)
plt.pause(1)
plt.clf()
ngauss = 11 
s = sectors_photometry(img, f.eps, f.theta, f.xpeak, f.ypeak,n_sectors=7,plot=1 ,minlevel = 0.01)
...
mge_print_contours(img, f.theta, xc, yc, m.sol,binning=10,scale=scale)
